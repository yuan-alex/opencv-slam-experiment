...
class MatchKeypoints(object):
    def __init__(self, prev = {"kp": None, "des": None, "frame": None}, bf = cv2.BFMatcher(cv2.NORM_HAMMING, crossCheck=True), matches = None):
        # Storing all previous stuffs in a dict
        self.prev = prev
        self.bf = bf
        self.matches = matches

    def store_as_previous(self, kp, des, frame):
...
class Draw(object):
    def __init__(self, custom_color = (0, 255, 0), new_frame = None, font = cv2.FONT_HERSHEY_SIMPLEX, current_frame_idx = None, prev_frame_idx = None, x1 = None, x2 = None, y1 = None, y2 = None):
    ...
    def matches(self, current_frame, prev_frame, matches, current_kp, prev_kp, current_des, prev_des):
		...
		# Only get the top 200 best matches. The matches list should be already sorted.
...
while True:
    ...
    if store_frame_counter % 8 == 0:
        ...
        MatchKeypoints.store_as_previous(kp, des, frame)
